...
export const contracts = sqliteTable(
  'contracts',
  {
    ...
    hasCustomization: integer('has_customization', { mode: 'boolean' }).notNull(),
    hasStagedAcceptance: integer('has_staged_acceptance', { mode: 'boolean' }).notNull(),
    ...
    anyPriorCommissionPaid: integer('any_prior_commission_paid', { mode: 'boolean' }).notNull(),
...
export const installments = sqliteTable(
  'installments',
  {
    ...
    milestoneComplete: integer('milestone_complete', { mode: 'boolean' }).notNull(),
    ...
    earlyPayment60Days: integer('early_payment_60_days', { mode: 'boolean' }).notNull(),
    earlyPaymentEvidence: text('early_payment_evidence'),
    deliveryAhead30Days: integer('delivery_ahead_30_days', { mode: 'boolean' }).notNull(),
    deliveryEvidence: text('delivery_evidence'),
    discountApprovalReference: text('discount_approval_reference'),
  },
  (table) => [
    uniqueIndex('idx_installments_contract_no').on(table.contractId, table.installmentNo),
